...
def ksm(delta):
    ...
    delta_ecdf = np.abs(delta_ecdf)
    return peaks


def get_signal_reactivity_peaks():
    for seq, fpath in acim_sequences.items():
        ...
        for read in acim['read_index'].unique():
            ...
            df = df.dropna()
            df['delta'] = np.subtract(df['event_level_mean_acim'], df['event_level_mean_dmso'])
        ...
        dfn[['read_index', 'position', 'contig', 'Predict', 'VARNA']].to_csv(save_path + seq + "_signal_peaks.csv",
                                                                             index=False)
        dfn = get_metric(dfn)
        dfn.to_csv(save_path + seq + "_signal_peaks_metric.csv", index=False)
        mx.get_Metric(dfn)
    return
